...
def save_novel_architecture(self):
    ...
    try:
        ...
        if not content:
            messagebox.showwarning("警告", "架构内容为空，无法保存")
            return
        ...
        if os.path.exists(filename):
            ...
            if saved_content.strip() == content:
                self.log("✅ 已成功保存对 Novel_architecture.txt 的修改。")
                messagebox.showinfo("成功", "架构修改已保存！")
            else:
        ...
        self.log(f"❌ 保存架构时出错: {str(e)}")
        messagebox.showerror("错误", f"保存架构时出错：{str(e)}")
