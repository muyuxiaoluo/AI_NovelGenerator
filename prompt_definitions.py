...
当前任务是：为【当前章节正文写作】生成一份**精简、克制、聚焦核心**的剧情大纲。
...
1. **结构要求：剧情关键帧（Key Plot Frames）**
   请将本章浓缩为 2-4 个核心事件点。
   **核心指令**：只写“发生了什么事”，不写“环境怎么样”或“心理怎么样”。
   格式如下：
   - **【节点一】**：[场景/地点]
     - [事件]：Who做了What（主谓宾结构）。
     - [看点]：本段的核心冲突、爽点或信息揭露。
   ...
   - **【节点二】**：...
2. **做减法原则**：
   - **合并琐事**：移动、等待、常规寒暄等过渡情节，请用一句话概括。
3. **逻辑一致性检查**（最高优先级！）：
   - **检查前文**：如果前文提到“连夜出发”，摘要中必须安排“夜间赶路”的情节，严禁直接跳到“次日清晨”。
   - **检查人设**：确认人物称呼与性别是否与前文一致（如：姜璃是女性）。
   - **逻辑自洽**：关注人物状态，不能出现“受伤的角色突然精力充沛”之类的违和点。
...
一、标点洁癖与视觉净化
1. **感叹号全面解放**：
    * **核心**：打破“只在对话用感叹号”的陈旧思想。**旁白/描写**只要带有情绪、决断或冲击力，**必须使用感叹号！**
    * **适用场景**：
        - **成功/结果**：“成功了！” / “完美剥离！”
        - **震惊/突变**：“竟然是它！” / “不好！”
        - **短句白描**：“快！” / “再快一点！”
    * **操作指令**：在关键节点的短句后，**严禁使用句号**，全部砸成**感叹号！**
2. **标点处理**：
    ** 引号大清洗 **
      1.  **引号白名单 (White List)**：
         * 双引号 `“”` **仅限**用于 **【人物说出的对白】**。
         * **除此之外，正文中严禁出现任何双引号！**
      2.  **绝对禁止强调 (No Emphasis)**：
         * 大模型习惯给“动词”、“特殊名词”、“形容词”加引号表示强调，**这是严令禁止的！**
         * **严禁**：轻轻一“点”、凝成一根“丝”、微小的“疏漏”、感知中的“凹点”。
         * **必须写成**：轻轻一点！凝成一根丝。微小的疏漏。感知中的凹点。
      3.  **视觉自检**：
         * 输出前请自查：如果这行字不是跟在“他说/道：”后面，就不该有引号。
         * **无省略号**：人物正常说话中不要使用省略号！被打断用破折号（——），思考用动作。
3. **排版切分**：
    * 段落**严禁超过 3 行**。
    * **移步必换行**：场景/视角/时间变化，必须换行。
    * **动言分离**：长动作（>20字）后，紧接的对话必须换行。

二、双轨制节奏与动态排版 (Rhythm & Layout)
1.  **旁白 (Narrative) -> 极简脆爽**：
    * **核心法则**：动作、环境、心理描写，**默认一句一段**。
    * **行数限制**：单段落**严禁超过 3 行**（手机端阅读体验）。
    * **去形容词**：封杀“仿佛”、“玄之又玄”等虚词。能用动词“砸/刺/撞”，就别用形容词“猛烈地”。
    * *场景自适应*：
        -   *战斗/突变*：句子极短（3-10字），多用感叹号。
        -   *氛围/铺垫*：允许稍长（15-30字），但必须有具体画面。

2.  **对话 (Dialogue) -> 声纹自适应 (Voice Adaptation)**：
    * **核心法则**：对话长度与风格**不设限**，必须根据【当前人物状态】动态调整。
    * **声纹建模 (Voice Modeling)**：
        -   **高位者/智者/掌控方**（如宗师、BOSS、布局者）：
            * *特征*：语速从容，允许**长难句**，多用反问、停顿（逗号），说完话常伴随慢动作（喝茶/拂袖）。
            * *示例*：“这世间道理，不是靠嘴说的。你若不服，尽管来试，只怕你付不起代价。”
        -   **低位者/急迫者/受压方**（如求生者、愤怒者、下属）：
            * *特征*：语速急促，**短句破碎**，多用感叹号，直奔主题，无废话。
            * *示例*：“快！那是陷阱！别去！”
    * **动言结合**：长对话中间必须插入**微动作**（Micro-Action）来控制呼吸感，禁止大段文字块。

三、POV 视角与环境执行
1.  **POV 防火墙**：严格限制在当前主角感官内。严禁写出当前人物此刻不知道的背景设定。
2.  **视觉锚点锁定**：环境描写必须**照抄知识库**的物体特征，**严禁二创**。
...
2. **对话节奏架构**：
    * **紧张场景**：短句为主，多用感叹号/破折号。
    * **信息交代**：长句必须拆分为“动作+对话”组合，禁止大段独白。
3. **潜台词系统**：
    * 拒绝“A说B说”。必须用**微动作/神态**暗示真实意图。
6. **拒绝“NPC式”对话**：
    * **禁令**：严禁两人像机器人一样交换信息（“我完成了。”“给你药。” -> **错！**）。
    * **操作**：对话必须包含**情绪**、**潜台词**或**身份差异**。

五、叙事节奏与蒙太奇
1.  **蒙太奇跳跃**：**封杀流水账**（走路/收拾）。上一段决定出发，下一段直接抵达。
2.  **动作驱动**：每 300 字至少包含一个具体动作。禁止纯对话超过 500 字。
3.  **心理节制**：闪回画面 ≤ 2 个，心理独白 ≤ 3 行。将心理转化为**具象的身体感受**（如喉咙发干）。
...
1.  **黄金段落**：
    ...
    * **中间**：每 1500 字一个小高潮（爆点）。
    * **结尾**：留悬念（接下来怎样）。
2.  **逻辑铁律**：用户指导 > 知识库。能力/物品/时间线必须前后一致。
...
请输出修改后的完整章节正文（不要包含任何解释性语言，直接输出正文）：
"""
