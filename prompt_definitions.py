...
# 8.0 本章出场角色提取（草稿写作前人物卡）
CHAPTER_CAST_PROMPT = """\
你是小说的“人物连续性编辑”。你的任务是在写作前，为本章生成一份**人物卡**，用于防止人物崩坏、关系写错、动机不一致、名字乱编。

【输入】
- 前情提要（长期记忆）：{global_summary}
- 上一章结尾（衔接锚点）：{previous_chapter_excerpt}
- 角色当前状态（最可信）：{character_state}
- 本章核心摘要（本章要做什么）：{short_summary}
- 作者隐式设定/补充指导（必须遵守）：{user_guidance}
- 本章要素：人物={characters_involved} / 道具={key_items} / 场景={scene_location}

【输出要求】只输出以下结构，不要解释：

1) 【本章出场角色清单】（严格筛选标准，按重要性排序，3-8人）
- **筛选铁律**：
  - **必须**是本章摘要或上一章结尾明确指出将“在场”的角色。
  - **必须**是本章场景中直接互动、有对话或直接行动的角色。
  - **严禁**列入仅在回忆、旁白提及、或仅存在于“关系网”但未出场的角色。
  - **严禁**列入仅作为背景板或无台词的龙套，除非其动作直接影响剧情。
- 角色名：一句话身份/当前状态关键词（必须用前文已有名字，不得编造）

2) 【关系网】（只列本章出场角色之间，使用箭头）
- A → B：关系/态度（例如：怀疑/信任/利用/仇恨/保护）

3) 【人物特点与动机】（每人2-4条，必须可执行、可写进剧情）
- 角色名：
  - 性格/行为习惯：…
  - 当前目标：…
  - 当前阻碍/风险：…
...
4) 【名字一致性警戒】（列出容易混淆的名字/称号/指代，若无写“无”）
"""
